...
describe Parser do
  ...
  it "parse the file correctly" do
    ...
    expect(first_parsed_class.method_calls.count).to eql(6)
    method_calls = first_parsed_class.method_calls
    expect(method_calls[1].class_name).to eql("C::D")
    expect(method_calls[1].method_name).to eql(:some_method)
    expect(method_calls[1].params[0]).to eql(:param1)
  end
